...
CHUNK_SIZE = 10.0  # chunk noises to a size of 10 seconds
########################### 1. Configurations
######### parse commands
parser = argparse.ArgumentParser("save noise to h5 file")
parser.add_argument(
    "--split", type=str, default="train", choices=["train", "test", "valid"]
)
...
output_h5_dir = "/fs/scratch/PAS0774/ashutosh/WSJ0_ATT_TRAIN/data/noise/{}kHz/".format(
    SRATE // 1000
)
...
def save_to_h5(count, n):
    filefolder = "{}-{}".format(
        (count // FOLDER_CAP) * FOLDER_CAP, (count // FOLDER_CAP + 1) * FOLDER_CAP - 1
    )
