...
######### file paths
input_wav_dir = "/fs/scratch/PAS0774/ashutosh/premix_data/LibriSpeech/"
output_h5_dir = "/fs/scratch/PAS0774/ashutosh/ATT_TRAIN/data/speech/{}kHz/".format(args.srate//1000)
meter = pyln.Meter(args.srate)

def save_to_h5(count, path):
    input_path = os.path.join(input_wav_dir, path)
    output_path = os.path.join(output_h5_dir, path.replace(".flac", ".samp"))
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    writer = h5py.File(output_path, "w")
    s, srate_ = sf.read(input_path)
    if srate_ != args.srate:
        s = resample_poly(s, args.srate, srate_)
