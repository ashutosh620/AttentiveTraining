...
print("offset is ", args.fix_offset, args.min_offset)


speech_dir = "/fs/scratch/PAS0774/ashutosh/ATT_TRAIN/data/speech/16kHz"
noise_dir = "/fs/scratch/PAS0774/ashutosh/ATT_TRAIN/data/noise/16kHz"
if args.split == "test":
    speech_list = "../filelists/test-clean_filelists.txt"
    noise_list = "../filelists/test_noise_list.txt"
elif args.split == "valid":
    speech_list = "../filelists/dev-clean_filelists.txt"
    noise_list = "../filelists/valid_noise_list.txt"

with open(speech_list, "r") as f:
    speech_list = [line.strip().replace(".flac", ".samp") for line in f]
...
overlap_type_list = ["none", "random", "half", "max"]
...
# sys.exit()
df.to_csv(
    "../filelists/{}_metadata_{:.1f}_{}.csv".format(
        args.split, args.min_offset, int(fix_offset)
    ),
    index=True,
    index_label="index",
)
